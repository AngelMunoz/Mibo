...
- **Adaptive:** the same audio surface as intent members on `IntentQueue` (`ctx.Intents.play(ctx.Context, "jump")`, plus the full music set), posting one closure that resolves the service at drain time — Headless-safe like every intent. `AdaptiveRaylibProgram.withBank` / `AdaptiveMonoGameProgram.withBank` declare the sound bank as one list value and load it before the program's `Init`, matching the MVU lane.
...
- **MonoGame 3D (DirectX 12):** skinned + instanced shadows no longer crash for models with more than 448 bones. Past that bone budget the shadow pass draws per instance, and a model whose mesh parts merge crashed that path. Merged models now draw per instance there, so shadows render.
