...
- `Mibo.MonoGame.Graphics3D`: scaffolding + core 3D types (`Conversions.fs`, `Light3D.fs`, `Material3D.fs`) and command-buffer infrastructure (`Command3D.fs`, `RenderBuffer3D.fs`, `RenderPipeline3D.fs`, `RenderTargetPool3D.fs`, `Renderer3D.fs`, `Draw3D.fs`). Internal `Conversions` module provides `System.Numerics`↔XNA bridge at the Core/backend boundary. `Material3D` is a minimal PBR-param struct with `fromBasicEffect`/`tryFromModelMeshPart` helpers for the native effect path. `Renderer3D<'Model>` owns a `RenderBuffer3D` + `RenderTargetPool3D` and delegates to a pluggable `IRenderPipeline3D`. Ships `NoopPipeline` as a placeholder. `Camera3D` + `Camera3DConfig` types added for MonoGame backend (pixel viewport, post-process pass selection).
