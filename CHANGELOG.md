...
- **Adaptive:** the same audio surface as intent members on `IntentQueue` — `ctx.Intents.play(ctx.Context, "jump")` plus the full music set — which play nothing without a registered audio service. `AdaptiveRaylibProgram.withBank` / `AdaptiveMonoGameProgram.withBank` declare the sound bank as one list value that loads before the program's `Init`.
