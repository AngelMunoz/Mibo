...
- `Mibo.MonoGame.Graphics3D.ForwardPipeline`: custom Cook-Torrance PBR pipeline (B9). `DrawMeshPBR` and `DrawMeshInstanced` now dispatch through the new `Shaders/ForwardPbr.fx` — ambient + 1 directional + up to 8 point + up to 4 spot lights (Cook-Torrance BRDF: GGX distribution, Smith geometry, Schlick Fresnel), emission, opacity, UV tiling, and optional normal-map TBN (re-derived in right-handed convention per §6.2). The single `.fx` exposes three techniques (Standard / Instanced / Skinned) sharing one fragment shader; the Skinned technique compiles but is not bound until B12. Uniforms are cached once on load (`PbrEffectParams`), and a `MaterialKey` short-circuit skips re-uploading material scalars + textures across consecutive draws sharing a material (mirrors the canonical raylib `MaterialKey`). Lights upload count-driven arrays (no zero-fill; the shader early-outs via `*Count`). `cameraPos` is uploaded unconditionally on every PBR draw (correctness fix vs. the canonical raylib pipeline, which only uploads it when shadow casters exist). When the PBR effect can't be loaded (missing embedded resource), both paths fall back gracefully — `DrawMeshPBR` to the B5/B6 `BasicEffect` albedo-only path, `DrawMeshInstanced` to the B7 minimal `Instanced.fx`. Per §4.1, PBR binds only on `PrimitiveMesh` (never `ModelMeshPart`).
- `Mibo.MonoGame.Shaders.ForwardPbr` (`.fx` + `.dx.mgfx`/`.ogl.mgfx`): Cook-Torrance PBR effect compiled to both DirectX 11 and OpenGL profiles. Pure PBR lighting is SM3.0-clean; light loops use `[loop]` + `if (i >= Count) break;` (OGL SM3.0 requirement per §6.3). Shadow sampling (`dFdx`/`dFdy`/`textureSize`) is deferred to B10.
- `Mibo.MonoGame.Shaders.DepthShadow` (`.fx` + `.dx.mgfx`/`.ogl.mgfx`): depth-only shadow-pass shader stub for B10 (directional shadow atlas). Created in B9 per plan; not bound in any dispatch path until B10.
...
- `Mibo.MonoGame.Graphics3D.ForwardPipeline`: PBR `normalMatrix` was uploaded inside the `MaterialKey`-gated block, so consecutive `DrawMeshPBR` calls sharing a material but with different transforms reused the first draw's normal matrix → wrong lighting. The `normalMatrix` is now uploaded unconditionally on every draw alongside `matModel`/`viewProj`/`cameraPos` (it is per-transform, not per-material), and the `normalMatrix` parameter was removed from `uploadPbrMaterial`.
- `Mibo.MonoGame.Shaders.ForwardPbr`: `VS_Instanced` transformed normals by the `normalMatrix` uniform (uploaded as Identity for instanced draws), which ignored per-instance rotation and left normals in model space. It now transforms normals by the per-instance `world` matrix directly (correct for uniform-scale instances: rotation is orthogonal, so inverse-transpose == world). Strictly better than the canonical raylib pipeline, which has the same latent limitation.
