...
- `Mibo.MonoGame`: 2D rendering stack (`Command2D`, `RenderBuffer2D`, `Draw` DSL, `Renderer2D`, `Camera2D`) — MonoGame port of the Raylib `Graphics2D` surface. Supports `FillRect`, `FillCircle`, `Sprite`, `Text`, camera transforms, and `DrawImmediate` escape hatch. The `Renderer2D` drives a `SpriteBatch` with state-transition flush semantics (camera changes end/re-begin the batch). Procedural textures (white pixel, radial circle) are generated lazily from the `GraphicsDevice` for shape fills. Multi-targets `net8.0;net10.0` (matching `Mibo.Raylib`).
