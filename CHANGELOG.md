...
- **MonoGame 3D (DX12):** skinned + instanced draws now work on the DX12 backend. VTF is unavailable there (the native runtime throws `NotSupportedException`), so bone palettes ride a `bonePaletteGroup[320]` constant array in an isolated `ForwardPbrGrouped.fx` — the mgfx DX12 reflection parser drops the params from the main `ForwardPbr.fx` when all 8 techniques are in one file. The shadow pass uses a matching `DepthShadowGrouped.fx`. Draws are chunked at 320/boneCount instances per group (was per-instance fallback — one draw per instance).
