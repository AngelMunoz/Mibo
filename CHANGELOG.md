...
- **Raylib 3D: extensible pipeline** — `ForwardPipelineBase` (abstract; owns the gather + frame orchestration + a virtual `Shade`) with `ForwardPbrPipeline` as the thin PBR subclass. Override `Shade` to plug a different shading strategy; it receives the per-frame scene (lights, shadow output, `time`). Register the same way: `Renderer3D.create (ForwardPbrPipeline()) view`.
