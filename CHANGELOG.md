...
- `Mibo.MonoGame.Camera3D`: full 3D camera module (`lookAt`, `orthographic`, `orbit`, `screenPointToRay`) with `Camera` (View + Projection) and `Ray` structs. Builders: `render`, `withViewport`, `withClear`, `withPostProcess`, `withoutPostProcess`, `splitScreenLeft`/`Right`/`Top`/`Bottom`, `overlay`. Uses XNA `Matrix.CreateLookAt`/`CreatePerspectiveFieldOfView`/`CreateOrthographic` in right-handed convention.
