...
- Shared 2D animation types in `Mibo.Core` (`Mibo.Elmish.Next.Animation`): `SpriteSheet`, `AnimatedSprite`, `Animation`, `GridAnimationDef`, `Point` with texture handles as `int<Texture>`. Backend-specific animation shims can register native textures and produce these neutral sheets.
- Neutral 2D camera helpers in `Mibo.Core` (`Mibo.Elmish.Next.Camera2D`): `create`, `viewportBounds`, `smoothFollow`, `clampTarget`, `render`, `withViewport`, `withClear`, and split-screen/overlay builders. These operate on `Camera2DState` and avoid colliding with the existing Raylib-specific `Mibo.Elmish.Camera2D` module.
- `Camera2DState` fields are now mutable so `byref` camera helpers can update the target without copying the whole struct.
- MonoGame backend now consumes the shared Core lighting/particle/camera/animation types. Its `LightContext2D`, `LightDraw`, and `State` modules were updated to use `Mibo.Elmish.Next.Graphics2D.Lighting` and `Mibo.Elmish.Next.Camera2D` types. `LightDraw.litAnimatedSprite` now accepts a `Mibo.Elmish.Next.Animation.AnimatedSprite` and resolves texture handles through the buffer registry.
- Backend-neutral 2D/3D render command DUs in `Mibo.Core` (`Mibo.Elmish.Next.Graphics2D`): `Command2D` (47 cases, struct DU) using opaque measure-tagged integer handles (`int<Texture>`, `int<Font>`, `int<Shader>`, `int<RenderTarget>`, `int<LightContext>`) and neutral primitives (`Color`, `Rect`, `BlendMode`, `Camera2DState`, `RenderLayer`). `Command3D` (18 cases) similarly with `int<Mesh>`, `int<ModelAsset>`, `MaterialData`, and neutral light types.
- Abstract render buffer bases in Core: `RenderBuffer2DBase` and `RenderBuffer3DBase` (abstract classes) with `ArrayPool`-backed storage, stable `int64` layer sort, and closed `getLayer` match. Backends subclass to carry resource registries.
- Neutral primitives in Core (`Mibo.Elmish.Next.Graphics2D.Base`): `Color` struct (RGBA byte, layout-compatible with `Raylib_cs.Color` and `Microsoft.Xna.Framework.Color`), `BlendMode` enum. Other primitives in `Mibo.Elmish.Next.Graphics2D`: `Rect`, `Camera2DState`, `Camera2DConfig`, `Camera`, `Camera3DConfig`, resource-handle measures (`Texture`, `Font`, `Shader`, `RenderTarget`, `Mesh`, `ModelAsset`, `LightContext`), `RenderLayer`.
- Raylib backend resource registries (`RaylibTextureRegistry`, `RaylibFontRegistry`, `RaylibShaderRegistry`, `RaylibRenderTargetRegistry`, `LightContextRegistry`, `RaylibMeshRegistry`, `RaylibModelRegistry`): forward map keyed by GPU id tokens (e.g. `Texture2D.Id`), reverse map as `ResizeArray` for O(1) resolution. Never keyed by raylib struct reflection equality.
