...
- **Mibo.Adaptive** — the adaptive-data library, adopted from AdaptiveSlop in its entirety and renamed (`AdaptiveSlop.Core` → `Mibo.Adaptive`). Dependency-free: the core never references Mibo; the Mibo integration (`AdaptiveProgram`/`AdaptiveHeadless`) lives in Mibo.Core, and `AdaptiveRaylibGame`/`AdaptiveGame` hosts ship in the raylib/MonoGame backends. Ships with its own changelog and versioning, an xunit + FsCheck test suite, and BenchmarkDotNet benchmarks. The AdaptiveSlop submodule is gone.
...
- **Adaptive (experimental):** windowed hosts — `AdaptiveRaylibGame` (raylib) and `AdaptiveGame` (MonoGame) run an adaptive world in a window with the MVU ceremony removed: no `Program` builder, no Cmd/Sub machinery, input registered and polled when opted in via `withInput`, services read straight from the context. A world can rebuild itself — the `RestartRequested` root and the runner's `Restart()` re-run `Init` for a fresh graph, fresh subscriptions and a reset clock.
