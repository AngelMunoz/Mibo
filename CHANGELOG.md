...
- **Adaptive (experimental):** `src/Defli` — the tower-defense sample (nine systems, four waves) ported from Defli's MVU shell onto `AdaptiveHeadless`, headless-first. The `World` record is the composition root: the per-frame router runs the systems in order and handles their events directly (the Cmd pump's same-frame ordering is preserved — the boss split and wave-fill stay synchronous), cold paths are plain handlers, and a `RenderFrame` struct packs every output projection once per `Step` for O(1) draw reads. `dotnet run --project src/Defli -- sim` plays four waves under an autonomous policy and prints per-projection recompute telemetry; the full Defli test suite (94 tests) passes against the port.
- **Adaptive (experimental):** `src/Defli.Raylib` — the windowed raylib frontend for the Defli port: the restored view layer reads only the forced `RenderFrame`, and the game's camera is now backend-neutral (`CameraState` — target/zoom/rotation/shake facts with pure, unit-tested view math), with the native raylib camera built at the view edge. `dotnet run --project src/Defli.Raylib` plays the game windowed (waves, placement, HUD, F3 diagnostics, R restart); the headless sim and its tests are unchanged.
- **Adaptive (experimental):** the windowed host shape — `AdaptiveRaylibGame<'Frame>` in `Mibo.Raylib` runs an adaptive world in a raylib window with the MVU ceremony removed: no `Program` builder, no Cmd/Sub machinery, input always registered and polled, and services read straight from the context in the world's `Init`/`Update` (the runner now accepts a pre-built `GameContext`; `AdaptiveContext` exposes it). A world can rebuild itself — `RestartRequested` root + the runner's `Restart()` re-run `Init` for a fresh graph, fresh subscriptions and a reset clock — which the host consumes after `Step` (the R-restart flow).
