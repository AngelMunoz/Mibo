...
- **MonoGame 3D, Raylib 3D:** skinned + instanced draws. `buffer.animatedModelInstanced(animModel, transforms, poses, ?material, ?colors)` draws N instances of the same animated model in one draw call (per sub-mesh), each instance with its own world transform and bone pose. `poses` carries one caller-evaluated `BonePose` per instance — compute poses once per frame and share them with bone queries and attachment draws. `material` is an optional `MaterialOverride` (`All` / `PerMesh`); `colors` (MonoGame only) tints each instance. Per-instance bone palettes are uploaded once per frame as a texture the vertex shader samples (raylib indexes it by `gl_InstanceID`), shared between the shadow and forward passes. On DX12 — where the native runtime has no vertex texture fetch — palettes ride a per-group constant array instead, and draws are chunked into groups of `320 / boneCount` instances (elsewhere, chunks of 2048). **OpenGL note:** the MonoGame OpenGL backend has no vertex texture fetch either, so there the draw falls back to per-instance skinned draws (correct, but no batching win — this includes Android). Mesh parts that share a parent bone, vertex layout, and material draw as one merged part per chunk automatically — the merged geometry is built lazily on a model's first instanced draw and cached per model — and a command whose materials split a group (e.g. a `PerMesh` override) falls back to per-part draws for that command, so output never changes. Custom effects in a `beginEffect` scope can opt in via a `SkinnedInstanced` technique (MonoGame) or the `bonePalette` sampler plus bone attributes (raylib) — see `docs/shader-uniforms.md`. See `docs/animation3d.md` → "Skinned + instanced draws".
...
- **Raylib 3D — Breaking (behavioral):** `AnimatedMesh.computeBoneMatrices` now returns the bone palette in plain System.Numerics row-major layout (`result[i] = InverseBindPose[i] * pose[i]`) instead of pre-transposed into raylib's native layout. The framework transposes at upload time where the shader contract needs it, so rendered output is unchanged — only code that reads the results directly and applies its own transpose needs to drop it.
