...
**Inside the feature** when it reads only that feature's own containers. Build it in the feature's `init`, store it on the model, done. The example uses a mutable class model (some features prefer that to records; see [Scaling](scaling.html)) plus one F# shorthand: `voption` is F#'s allocation-free option type:
