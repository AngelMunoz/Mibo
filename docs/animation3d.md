...
Crossfade between two animations with `Animation3DState.blendTo` (this is Mibo's own API; raylib's low-level `UpdateModelAnimationEx` does the same job on raw models):
...
Skinned + instanced draws are supported (per-instance bone palettes ride a palette texture, see above), but attachments are not part of that path: an animated model with attachments costs one skinned draw plus one plain-mesh draw per attachment, per instance.
