...
| `Effect` | (none) | `Effect` | MonoGame: compiled `.mgfx` |
| `ModelAnimations` | `ModelAnimation[]` | `Animation3DClips` | Skeletal animation clips |
| `AnimatedMesh` | (none) | `AnimatedMesh voption` | MonoGame: loaded via Assimp at runtime |
...
let config = cache.GetOrCreate("gameConfig", loadConfig)
```
