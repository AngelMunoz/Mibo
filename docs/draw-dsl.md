...
let applyColorPass (pp: PostProcessContext2D) =
    // runs once after the scene renders; draw a fullscreen quad of pp.Source
    ()

let runRawBackendCalls () =
...
| --- | --- | --- |
| `layer` | `0<RenderLayer>` | 2D draw ordering |
...
- **Vectors and matrices**: `System.Numerics` (`Vector2`, `Vector3`, `Matrix4x4`). raylib uses these natively; the MonoGame <abbr title="witness: F# jargon for the implicit conversion functions the compiler inserts">witnesses</abbr> convert through the same `op_Implicit`/`.ToNumerics()` conversions MonoGame already ships.
...
Anything that *is* a backend type passes through as that type; there is no wrapper and no conversion. This is what lets one DSL cover both backends:
...
Sprites use the backend `SpriteState` record; text has both the record form and a parts form that skips the builder chain:
...
buffer
  ...
  .postProcess(applyColorPass)

  .drawImmediate(runRawBackendCalls)
  .drop()
```
...
```fsharp
let ghostMaterialAt (i: int) = ghostMaterials[i]
let colorOnlyPass (pp: PostProcessContext3D) = (* color-only pass *)
let depthBasedPass (pp: PostProcessContext3D) = (* fog, DOF, SSAO *)
...
buffer
  ...
  .modelWithPerMesh(ghostModel, ghostTransform, ghostMaterialAt)
  ...
  .endCamera()
  .postProcess(colorOnlyPass)
  .postProcessWithDepth(depthBasedPass)
  .drop()
```
...
Some features exist on one backend only. The DSL keeps them in the same fluent surface; they have no counterpart on the other backend, so **calling them there is a compile error** naming the missing operation:
