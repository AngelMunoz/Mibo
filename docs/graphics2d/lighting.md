...
Mibo includes a GPU-driven 2D lighting system with soft shadows using analytic Signed Distance Field (SDF) raymarching: shadows are computed per pixel by marching toward the light through a distance field of the occluders. No shadow map textures, no extra render passes. (Available on both backends.)
...
2. Each frame: `model.Lighting.Reset()` at the start of your view
...
let myView (ctx: GameContext) (model: Model) (buffer: RenderBuffer2D) =
    ...
    buffer
      ...
      // 3. Add a directional light (sun), built from parts...
      .addDirectionalLight(
        model.Lighting,
        Vector2(0.3f, -0.7f),
        Color.White,
...
- `Edge.Bottom ||| Edge.Left ||| Edge.Right`: platformers (skip top edge so the player can stand on it without self-shadowing)
...
- Lit sprites are batched: consecutive lit sprites sharing the same texture (and <abbr title="a texture that stores per-pixel surface direction, used to fake detail in lighting">normal map</abbr>) collapse into a single draw call rather than one per sprite. Group lit sprites by texture in your view to get the most out of this.
...
- [Buffer & Commands](buffer-and-commands.html): The buffer lifecycle and layer model
