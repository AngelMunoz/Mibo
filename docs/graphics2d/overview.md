...
| Custom GPU work (e.g. raw rlgl meshes, instancing) | Use `.drawImmediate(...)` (escape hatch). rlgl is raylib's raw graphics layer; instancing draws many copies of one mesh in a single GPU call |
...
1. `buffer.Clear()`: wipe previous frame's commands (the renderer clears the buffer for you at the start of each frame; do not call `Clear` yourself from your view)
...
The 2D lighting system (`Mibo.Elmish.Graphics2D.Lighting`) provides point lights, directional lights, ambient light, and <abbr title="signed distance field: shadows computed from a distance field of the occluders, giving soft edges">SDF</abbr> soft shadows, all GPU-driven with no extra render passes.
