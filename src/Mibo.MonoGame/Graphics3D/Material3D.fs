...
module Material3D =
  ...
  /// <summary>
  /// Reads a <c>BasicEffect</c> into a <see cref="T:Mibo.Elmish.Graphics3D.Material3D"/>.
  /// Only available when the effect is a <c>BasicEffect</c>;
  /// for custom effects the pipeline should bind its own shader.
  /// </summary>
  let fromBasicEffect(effect: BasicEffect) : Material3D =
    let diffuse = effect.DiffuseColor

    let albedoMap =
      match effect.Texture with
      | null -> ValueNone
      | tex -> ValueSome tex

    {
      AlbedoColor =
        Color(
          int(diffuse.X * 255.0f),
          int(diffuse.Y * 255.0f),
          int(diffuse.Z * 255.0f)
        )
      AlbedoMap = albedoMap
      Roughness = 0.5f
      RoughnessMap = ValueNone
      ...
      NormalMap = ValueNone
      EmissionColor =
        let em = effect.EmissiveColor
        Color(int(em.X * 255.0f), int(em.Y * 255.0f), int(em.Z * 255.0f))
      EmissionMap = ValueNone
  ...
  /// <summary>
  /// Attempts to read a <c>ModelMeshPart</c>'s effect as a <c>Material3D</c>.
  /// Returns <c>None</c> if the mesh part has no effect or if the effect
  /// is not a <c>BasicEffect</c>.
  /// </summary>
  let tryFromModelMeshPart(part: ModelMeshPart) : Material3D voption =
    match part.Effect with
    | :? BasicEffect as be -> ValueSome(fromBasicEffect be)
    | _ -> ValueNone
