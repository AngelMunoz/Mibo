...
let materialKeyTests =
  testList "MaterialKey.fromMaterial3D" [
    test "defaults produce key with zero map ids and default values" {
      let mat = Material3D.defaults
      let key = MaterialKey.fromMaterial3D &mat
      Expect.equal key.AlbedoMapId 0u "AlbedoMapId should be 0"
      Expect.equal key.RoughnessMapId 0u "RoughnessMapId should be 0"
      Expect.equal key.MetallicMapId 0u "MetallicMapId should be 0"
      Expect.equal key.NormalMapId 0u "NormalMapId should be 0"
      Expect.equal key.EmissionMapId 0u "EmissionMapId should be 0"
      Expect.equal key.AlbedoColor Color.White "AlbedoColor should be White"
      Expect.equal key.Roughness 0.5f "Roughness should be 0.5"
      Expect.equal key.Metallic 0.0f "Metallic should be 0"
      Expect.equal key.Opacity 1.0f "Opacity should be 1"
    }

    test "colored material preserves color in key" {
      let mat = Material3D.colored Color.Red
      let key = MaterialKey.fromMaterial3D &mat
      Expect.equal key.AlbedoColor Color.Red "AlbedoColor should be Red"
      Expect.equal key.EmissionColor Color.Black "EmissionColor should be Black"
    }

    test "different materials produce different keys" {
      let mat1 = Material3D.colored Color.Red
      let mat2 = Material3D.colored Color.Blue
      let key1 = MaterialKey.fromMaterial3D &mat1
      let key2 = MaterialKey.fromMaterial3D &mat2
      Expect.notEqual key1 key2 "Keys should differ for different colors"
    }

    test "same materials produce equal keys" {
      let mat1 = Material3D.colored Color.Red
      let mat2 = Material3D.colored Color.Red
      let key1 = MaterialKey.fromMaterial3D &mat1
      let key2 = MaterialKey.fromMaterial3D &mat2
      Expect.equal key1 key2 "Keys should be equal for same material"
    }

    test "custom roughness and metallic reflected in key" {
      let mat = {
      ...
      let key = MaterialKey.fromMaterial3D &mat
      Expect.equal key.Roughness 0.8f "Roughness should be 0.8"
      Expect.equal key.Metallic 0.3f "Metallic should be 0.3"
    }

    test "custom tiling reflected in key" {
      let mat = {
        Material3D.defaults with
            Tiling = Vector2(3.0f, 4.0f)
      }

      let key = MaterialKey.fromMaterial3D &mat
      Expect.equal key.TilingX 3.0f "TilingX should be 3"
      Expect.equal key.TilingY 4.0f "TilingY should be 4"
    }
  ]

let colorConversionTests =
  testList "colorToVec3 / colorToVec4" [
    test "colorToVec3 white" {
      let v = colorToVec3 Color.White
      Expect.floatClose Accuracy.medium (float v.X) 1.0 "R"
      Expect.floatClose Accuracy.medium (float v.Y) 1.0 "G"
      Expect.floatClose Accuracy.medium (float v.Z) 1.0 "B"
    }

    test "colorToVec3 black" {
      let v = colorToVec3 Color.Black
      Expect.floatClose Accuracy.medium (float v.X) 0.0 "R"
      Expect.floatClose Accuracy.medium (float v.Y) 0.0 "G"
      Expect.floatClose Accuracy.medium (float v.Z) 0.0 "B"
    }

    test "colorToVec3 red" {
      let c = Color(255uy, 0uy, 0uy, 255uy)
      let v = colorToVec3 c
      Expect.floatClose Accuracy.medium (float v.X) 1.0 "R"
      Expect.floatClose Accuracy.medium (float v.Y) 0.0 "G"
      Expect.floatClose Accuracy.medium (float v.Z) 0.0 "B"
    ...
    test "colorToVec4 white opaque" {
      let v = colorToVec4 Color.White
      Expect.floatClose Accuracy.medium (float v.X) 1.0 "R"
      Expect.floatClose Accuracy.medium (float v.Y) 1.0 "G"
      Expect.floatClose Accuracy.medium (float v.Z) 1.0 "B"
      Expect.floatClose Accuracy.medium (float v.W) 1.0 "A"
    }

    test "colorToVec4 preserves alpha" {
      let c = Color(255uy, 0uy, 0uy, 128uy)
      let v = colorToVec4 c
      Expect.floatClose Accuracy.medium (float v.W) (128.0 / 255.0) "A"
    }
  ]

let computeNormalMatrixTests =
  testList "computeNormalMatrix" [
    test "identity produces identity" {
      let nm = NormalMatrixHelpers.computeNormalMatrix Matrix4x4.Identity
      Expect.floatClose Accuracy.medium (float nm.M11) 1.0 "M11"
      Expect.floatClose Accuracy.medium (float nm.M22) 1.0 "M22"
      Expect.floatClose Accuracy.medium (float nm.M33) 1.0 "M33"
      Expect.floatClose Accuracy.medium (float nm.M44) 1.0 "M44"
    }

    test "uniform scale produces inverse scale" {
      let scale = Matrix4x4.CreateScale(2.0f)
      let nm = NormalMatrixHelpers.computeNormalMatrix scale
      Expect.floatClose Accuracy.medium (float nm.M11) 0.5 "M11"
      Expect.floatClose Accuracy.medium (float nm.M22) 0.5 "M22"
      Expect.floatClose Accuracy.medium (float nm.M33) 0.5 "M33"
    }
  ]

let clearLightsTests =
  testList "clearLights" [
    test "clears all light buffers" {
      let lights = createLightBuffers(8, 4)
      lights.Ambient.Add(AmbientLight3D.create Color.White)
      lights.DirLights.Add(DirectionalLight3D.create v3a)
      lights.PointLights.Add(PointLight3D.create(v3a, 10.0f))
      lights.SpotLights.Add(SpotLight3D.create(v3a, v3b, 10.0f))
      clearLights lights
      Expect.equal lights.Ambient.Count 0 "Ambient should be empty"
      Expect.equal lights.DirLights.Count 0 "DirLights should be empty"
      Expect.equal lights.PointLights.Count 0 "PointLights should be empty"
      Expect.equal lights.SpotLights.Count 0 "SpotLights should be empty"
    }
  ]

let handleLightCommandTests =
  testList "handleLightCommand" [
    test "AddDirectionalLight adds and marks dirty" {
      let lights = createLightBuffers(8, 4)
      let mutable dirty = false
      let light = DirectionalLight3D.create v3a
      handleLightCommand(lights, Command3D.addDirectionalLight light, &dirty)
      Expect.equal lights.DirLights.Count 1 "Should have 1 directional light"
      Expect.isTrue dirty "Should be dirty"
    }

    test "AddPointLight adds and marks dirty" {
      let lights = createLightBuffers(8, 4)
      let mutable dirty = false
      let light = PointLight3D.create(v3a, 10.0f)
      handleLightCommand(lights, Command3D.addPointLight light, &dirty)
      Expect.equal lights.PointLights.Count 1 "Should have 1 point light"
      Expect.isTrue dirty "Should be dirty"
    }

    test "AddSpotLight adds and marks dirty" {
      let lights = createLightBuffers(8, 4)
      let mutable dirty = false
      let light = SpotLight3D.create(v3a, v3b, 10.0f)
      handleLightCommand(lights, Command3D.addSpotLight light, &dirty)
      Expect.equal lights.SpotLights.Count 1 "Should have 1 spot light"
      Expect.isTrue dirty "Should be dirty"
    }

    test "SetAmbientLight replaces and marks dirty" {
      let lights = createLightBuffers(8, 4)
      lights.Ambient.Add(AmbientLight3D.create Color.Red)
      let mutable dirty = false
      let light = AmbientLight3D.create Color.Blue
      handleLightCommand(lights, Command3D.setAmbientLight light, &dirty)
      Expect.equal lights.Ambient.Count 1 "Should have 1 ambient light"
      Expect.equal lights.Ambient[0].Color Color.Blue "Color should be Blue"
      Expect.isTrue dirty "Should be dirty"
    }

    test "non-light command is no-op" {
      let lights = createLightBuffers(8, 4)
      let mutable dirty = false
      handleLightCommand(lights, Command3D.enableShadows(), &dirty)
      Expect.equal lights.Ambient.Count 0 "Should be unchanged"
      Expect.isFalse dirty "Should not be dirty"
    }
  ]
...
let tests =
  testList "Graphics3D" [
    ...
    computeNormalMatrixTests
    clearLightsTests
    handleLightCommandTests
    preScanTests
  ]
