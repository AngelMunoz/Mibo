...
// no GL context is needed and these run headless in CI.
let private dummyMesh() : Raylib_cs.Mesh = Unchecked.defaultof<_>
let private dummyMat() : Material3D = Unchecked.defaultof<_>
let private dummyShader() : Raylib_cs.Shader = Unchecked.defaultof<_>

let private cmdTag(cmd: Command3D) : string =
  match cmd with
  | Command3D.BeginEffect _ -> "begin"
  | Command3D.EndEffect -> "end"
  | Command3D.DrawMeshInstanced _ -> "draw"
  | _ -> "other"
...
// Context builders — keep test bodies free of multi-line named-arg indentation pitfalls.
// Named args disambiguate the two InstancedRenderContext ctors (primary vs triple overload).
let private pairsFor(meshCount: int) : struct (Raylib_cs.Mesh * Material3D)[] = [|
  for _ in 1..meshCount -> struct (dummyMesh(), dummyMat())
|]

let private ctxFromPairs meshCount =
  let getMeshesAndMaterial: int -> struct (Raylib_cs.Mesh * Material3D)[] =
    fun _ -> pairsFor meshCount

  InstancedRenderContext<int, int>(
    getKey = id,
    getMeshesAndMaterial = getMeshesAndMaterial,
    getTransform = fun _ _ -> Matrix4x4.Identity
  )

let private ctxFromTriples triples =
  let getMeshesMaterialAndShader
    : int -> struct (Raylib_cs.Mesh * Material3D * Raylib_cs.Shader voption)[] =
    fun _ -> triples
  ...
  testList "Layout3D" [
    ...
    testList "InstancedRenderer3D" [
      ...
      <| fun _ ->
        ...
        let triples = [|
          struct (dummyMesh(), dummyMat(), ValueSome(dummyShader()))
        ...
        // ValueSome sub-mesh wrapped; ValueNone sub-mesh not.
        Expect.equal
          (cmdSequence buf)
          [| "begin"; "draw"; "end"; "draw" |]
          "per-sub-mesh scope"
      ...
      <| fun _ ->
        ...
        let grid =
          CellGrid3D.create 2 1 1 (Vector3(1f, 1f, 1f)) Vector3.Zero
          |> Layout3D.run(fun s ->
            s |> Layout3D.set 0 0 0 0 |> Layout3D.set 1 0 0 1)

        let shaderForKey k =
          match k with
          | 1 -> ValueSome(dummyShader())
          | _ -> ValueNone
        ...
        // Exactly one contiguous [begin;draw;end] span and one [draw] span,
        // in some order. Count command kinds to assert the multiset.
      ...
      <| fun _ ->
        // The primary risk flagged in the plan: SRTP resolution of a witness
        // defined in a module-augmentation in Layout3D/Renderer3D.fs. If this
        // compiles AND runs, the cross-file witness resolves.
        use buf = new RenderBuffer3D()
        let ctx = ctxFromPairs 1
        let grid = singleCellGrid 0
      ...
      <| fun _ ->
        ...
        buf
          .renderCellGridInstanced(ctx, grid, fun _ -> ValueSome(dummyShader()))
          .drop()
