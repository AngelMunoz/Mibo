...
let update (boxRef: ComponentRef<InteractiveBoxOverlay>) (msg: Msg) (model: Model) =
    match msg with
    | Tick gt ->
    ...
    | DemoBoxBounced count ->
        ...
        let struct (newParticles, _) =
            Particles.update (Particles.Emit(model.Player.Player.Position, 20)) model.Particles
