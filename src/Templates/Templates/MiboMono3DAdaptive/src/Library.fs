...
let view (ctx: GameContext) (frame: Frame) (buffer: RenderBuffer3D) =
  let primSet =
  ...
  let transform = Matrix.CreateTranslation(frame.Position)
...
let create() : AdaptiveMonoGameProgram<Frame> =
  ...
  |> AdaptiveProgram.withConfig(
    ...
    >> GameConfig.withTitle "Mibo MonoGame 3D Adaptive Game"
  )
