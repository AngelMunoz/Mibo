# AGENTS.md: Mibo Raylib 3D Adaptive Game
...
and level layout, input mapping, lighting and shadows, GPU instancing, skeletal
animation, incremental derived state, a deferred command-buffer renderer, and
more. **Before creating a new sub-module, check the docs**; the building
block you are about to write likely already exists. Compose existing pieces;
...
> post-processing are already available; don't re-implement a rendering
> strategy. The view reads the packed `Frame` and fills a `RenderBuffer3D`.
