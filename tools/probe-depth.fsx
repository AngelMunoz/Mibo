...
elementJoins <- 0; joinMaps <- 0; alivePreds <- 0; countComputes <- 0; bannerComputes <- 0
...
let totalShape1 = elementJoins + joinMaps + alivePreds + countComputes + bannerComputes
printfn "── Shape 1 (game's chain): one frame, two writes (pos+health of enemy 7) ──"
printfn "element joins=%d tryFind joins=%d alive preds=%d count=%d banner=%d → total %d node recomputes"
    elementJoins joinMaps alivePreds countComputes bannerComputes totalShape1
...
printfn "graph node recomputes=%d (plain dictionary reads)" (elementJoins + joinMaps + alivePreds + countComputes + bannerComputes)
