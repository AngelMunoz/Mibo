...
let t =
    data
    |> AMap.tryFind 3
    |> AVal.map(fun v -> transforms <- transforms + 1; v)
...
printfn "unrelated key write → transform ran %d time(s) (naive expectation: 0)" transforms

// 2) the WATCHED key written with an EQUAL value (source equality filter)
transforms <- 0
CMap.addOrUpdate 3 30 data // 30 == 3*10, equal
AVal.getValue t |> ignore
printfn "equal-value write   → transform ran %d time(s) (expect 0 — source filter)" transforms

// 3) the watched key REMOVED (presence flips)
transforms <- 0
CMap.remove 3 data
AVal.getValue t |> ignore
printfn "key removed          → transform ran %d time(s) (expect 1 — presence changed)" transforms
